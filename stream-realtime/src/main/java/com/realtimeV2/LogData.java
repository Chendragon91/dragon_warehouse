...
public class LogData {
    private static final String KAFKA_IP = ConfigUtils.getString("kafka.bootstrap.servers");
    private static final String KAFKA_LOG_DATA_TOPIC = ConfigUtils.getString("REALTIME.KAFKA.LOG.TOPIC.V2");
    ...
    private static final OutputTag<String> pageTag =new OutputTag<String>("pageTag"){};
    ...
    public static void main(String[] args) throws Exception {
        ...
        dirtyDs.sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_DIRTY_TOPIC))
    ...
    public static void SplitDs2kafkaTopicMsg(Map<String,DataStream<String>> dataStreamHashMap){
        dataStreamHashMap.get("errTag").sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_ERR_LOG))
                .uid("sk_errMsg2Kafka")
                .name("sk_errMsg2Kafka");

        dataStreamHashMap.get("startTag").sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_START_LOG))
                .uid("sk_startMsg2Kafka")
                .name("sk_startMsg2Kafka");

        dataStreamHashMap.get("displayTag").sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_DISPLAY_LOG))
                .uid("sk_displayMsg2Kafka")
                .name("sk_displayMsg2Kafka");

        dataStreamHashMap.get("actionTag").sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_ACTION_LOG))
                .uid("sk_actionMsg2Kafka")
                .name("sk_actionMsg2Kafka");

        dataStreamHashMap.get("page").sinkTo(KafkaUtils.buildKafkaSink(KAFKA_IP, KAFKA_PAGE_TOPIC))
