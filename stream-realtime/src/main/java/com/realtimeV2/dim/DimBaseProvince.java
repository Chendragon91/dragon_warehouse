...
public class DimBaseProvince {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String groupId = "dim_base_province_group";

        tEnv.executeSql(
                "CREATE TABLE ods_professional (" +
                        "  op STRING," +
                        "  `after` ROW<id BIGINT, name STRING, region_id BIGINT, area_code STRING, iso_code STRING>," +
        ...
        tEnv.executeSql(
                "CREATE TABLE dim_base_province (" +
                        "  rowkey STRING," +
                        "  info ROW<name STRING, region_id STRING, area_code STRING, iso_code STRING>," +
        ...
        tEnv.executeSql(
                "INSERT INTO dim_base_province " +
                        "SELECT CAST(`after`.id AS STRING), " +
                        "       ROW(`after`.name, CAST(`after`.region_id AS STRING), `after`.area_code, `after`.iso_code) " +
                        "FROM ods_professional " +
                        "WHERE op IN ('c','u','r') " +
                        "  AND source.`table`='base_province' " +
