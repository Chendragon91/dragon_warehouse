...
public class DimSpuInfo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String groupId = "dim_spu_info_group";

        tEnv.executeSql(
                "CREATE TABLE ods_professional (" +
                        "  op STRING," +
                        "  `after` ROW<id BIGINT, spu_name STRING, description STRING, tm_id BIGINT, category3_id BIGINT, create_time BIGINT>," +
        ...
        tEnv.executeSql(
                "CREATE TABLE dim_spu_info (" +
                        "  rowkey STRING," +
                        "  info ROW<spu_name STRING, description STRING, tm_id STRING, category3_id STRING, create_time STRING>," +
        ...
        tEnv.executeSql(
                "INSERT INTO dim_spu_info " +
                        "SELECT CAST(`after`.id AS STRING), " +
                        "       ROW(`after`.spu_name, `after`.description, CAST(`after`.tm_id AS STRING), CAST(`after`.category3_id AS STRING), CAST(`after`.create_time AS STRING)) " +
                        "FROM ods_professional " +
                        "WHERE op IN ('c','u','r') " +
                        "  AND source.`table`='spu_info' " +
    ...
    private static void createNamespaceIfNotExists(Admin admin, String namespace) throws IOException {
        try { admin.getNamespaceDescriptor(namespace); }
