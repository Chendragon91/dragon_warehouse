...
public class DimBaseCategory3 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String groupId = "dim_base_category3_group";

        tEnv.executeSql(
                "CREATE TABLE ods_professional (" +
                        "  op STRING," +
                        "  `after` ROW<id BIGINT, name STRING, category2_id BIGINT>," +
        ...
        tEnv.executeSql(
                "CREATE TABLE dim_base_category3 (" +
                        "  rowkey STRING," +
        ...
        tEnv.executeSql(
                "INSERT INTO dim_base_category3 " +
                        "SELECT CAST(`after`.id AS STRING), " +
                        "       ROW(`after`.name, CAST(`after`.category2_id AS STRING)) " +
                        "FROM ods_professional " +
                        "WHERE op IN ('c','u','r') " +
                        "  AND source.`table`='base_category3' " +
